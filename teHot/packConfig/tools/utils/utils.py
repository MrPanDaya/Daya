...
function get_nativeInfo()
{
    return return {version:%d,ios_no_h5:true}
}
get_nativeInfo()
'''

NATIVE_VERSION = 5
def write_native_info(file_path, ios_has_h5=False):
    f = open(file_path, 'w')
    if ios_has_h5:
        f.write(NATIVE_INFO_FMT_HAS_H5 % NATIVE_VERSION)
    else:
        f.write(NATIVE_INFO_FMT % NATIVE_VERSION)
    f.close()
