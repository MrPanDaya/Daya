...
# 获取文件的 hash 值，hashType 可选值 md5，sha1
def get_file_hash(filePath, isBinary, hashFunc):
    if not os.path.isfile(filePath):
        raise_known_error('文件 %s 不存在' % filePath, KnownError.ERROR_PATH_NOT_FOUND)
    ...
    if not cmd:
        raise_known_error('未知的 hash 类型： %s' % hashFunc, KnownError.ERROR_WRONG_ARGS)
...
def unzip(zip_file_path, folder_path):
    file = zipfile.ZipFile(zip_file_path.decode('utf-8'), 'r')
...
NATIVE_INFO_PRE_STR = '''
function get_nativeInfo()
{
    return {
'''
NATIVE_INFO_POST_STR = '''    }
}
get_nativeInfo()
'''
NATIVE_VERSION = 16
def write_native_info(file_path, params=None):
    cur_time_str = datetime.datetime.fromtimestamp(time.time()).strftime('%Y%m%d%H%M%S')
    final_params = {
        'version' : NATIVE_VERSION,
        'pkg_timestamp' : '"%s"' % cur_time_str
    }
    ...
    info_str = info_str + NATIVE_INFO_POST_STR
